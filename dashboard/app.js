...
async function loadDashboardData() {
  const candidates = [
    {
      reportPath: "../spotify_library_audit_report.json",
      artistsPath: "../ACTUAL SPOTIFY ARTISTS.json",
      mode: "live",
    },
    {
      reportPath: "../site/spotify_library_audit_report.json",
      artistsPath: "../site/ACTUAL SPOTIFY ARTISTS.json",
      mode: "demo",
    },
  ];
  ...
  for (const candidate of candidates) {
    try {
      const [report, artists] = await Promise.all([
        loadJson(candidate.reportPath),
        loadJson(candidate.artistsPath),
      ]);
      return { report, artists, mode: candidate.mode };
    } catch (error) {
      lastError = error;
    }
  }

  throw lastError || new Error("Не удалось загрузить данные дашборда");
}
...
function applyDataMode(mode) {
  ...
  if (mode === "demo") {
    eyebrow.textContent = "yaMusicToSpotify • Dashboard Demo";
    $("#heroStatus").textContent = "Demo";
    $("#heroStatusText").textContent = "Bundled sample data for the public repository";
  } else {
    eyebrow.textContent = "yaMusicToSpotify • Taste Dashboard";
  }
}
